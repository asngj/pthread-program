...
static void    gpio_reset1(int port, int bit);                   
 static void    gpio_set1(int port, int bit);     
         ...
         uint16_t datalen)
{
  printf("Data received on port %d from port %d with length %d, '%s'\n",
	...
	if(data == '1')
     { 
        gpio_set1(GPIO_A_BASE, (1 << 5) );
      }
      else
     {
          gpio_reset1(GPIO_A_BASE, (1 << 5) );
     }
...
/*
	 if(data == 1)
    { 
       gpio_set1(GPIO_A_BASE, (1 << 2) );
     }
     else
    {
         gpio_reset1(GPIO_A_BASE, (1 << 2) );
	}
*/
    //PROCESS_WAIT_UNTIL(etimer_expired(&periodic_timer));
  }
